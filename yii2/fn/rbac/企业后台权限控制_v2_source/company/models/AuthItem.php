...
class AuthItem extends \yii\db\ActiveRecord
{
    ...
    const SCENE_ROLE_AUTH = 3;
    const SCENE_ROLE_AUTH_EDIT =4;
    ...
    {
        $s = parent::scenarios();
        $s[self::SCENE_ROLE_AUTH] = ['role_name','role_desc','role_auth_id'];
        $s[self::SCENE_ROLE_AUTH_EDIT] = ['role_name','role_desc','role_auth_id','role_id'];
        return $s;
    }
    ...
    public function rules()
    {
        return [
            ...
            [['description'], 'string'],
            [['name', 'url'], 'string', 'max' => 10],
            [['role_name','role_auth_id'],'required','on'=>[self::SCENE_ROLE_AUTH,self::SCENE_ROLE_AUTH_EDIT]],
            [['role_name','role_desc'],'filter','filter'=>'trim','on'=>[self::SCENE_ROLE_AUTH,self::SCENE_ROLE_AUTH_EDIT]],
            ['role_name','string','min'=>1,'max'=>10,'on'=>[self::SCENE_ROLE_AUTH,self::SCENE_ROLE_AUTH_EDIT]],
            [['role_desc'],'string','max'=>2048,'on'=>[self::SCENE_ROLE_AUTH,self::SCENE_ROLE_AUTH_EDIT]],
            ['role_id','required','on'=>[self::SCENE_ROLE_AUTH_EDIT]],
            ['role_id','number','on'=>[self::SCENE_ROLE_AUTH_EDIT]]
        ];
    }
...
/*     public function beforeSave($insert)
     ...
     * @param array    $permissions    权限，多个权限则逗号隔开
     * @param int       $id         角色id，只有type=2的时候才用上
     * @return bool
     */
    public function editAuth ($type = 1, $permissions = [], $id = '') {
        $trans = null;
        if ($type == 1) {
            try {
...
//                    foreach ($arr_permission as $k=>$v) {
                ...
                $this->addError('role_name',$e->getMessage());
                $trans->rollback();
                return false;
            }
        } elseif ($type == 2) {
