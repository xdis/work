...
class User extends ActiveRecord implements IdentityInterface
{
     ...
     */
    public function afterSignup(array $profileData = [], array $companyData = [])
    {
        $this->refresh();
        ...
        if ($this->user_type == self::USER_TYPE_COMPANY) {
            ...
            if ($this->companyInfo && is_object($this->companyInfo)) {
                ...
                throw new \Exception("Company couldn't be  created");
            }
...
//        $auth->assign($auth->getRole(User::ROLE_USER), $this->getId());
     ...
     * @return array
     */
    public static function getCompanyUserTypeV2($type,$id,$name)
    {
        $company_id = Yii::$app->user->getCompanyId();
        if($type==1){ //查供应商
            $where['cus_company_id'] = $company_id;
        ...
        $data =  Company::find()->leftJoin('user AS u','u.id = company.user_id')
                                ->select("company.id,company.name as company_name,company.audit_status,u.username,company.address,
                                company.delegate_name,company.delegate_idcard,company.business_license,company.city_name as city_id,
                                (select name from region where id = company.city_name) as city_desc,
                                (select GROUP_CONCAT(name) from  lookup where code in((select industry_app_id from
                                company_app where company_id = company.id)) and type ='industry_appid' and code<>7) as type")
                                ->where(['u.user_type'=>self::USER_TYPE_COMPANY,'company.audit_status'=>1])
                                ->andWhere(['not in','company.id',$ids])
                                ->andWhere(['<>','company.id',$company_id])
                                ->andFilterWhere(['like','company.name',$name])
                                ->asArray()->all();
        return $data;
//        foreach ($data as $val) {
//            $select_data[$val['id']] = ['company_name'=>$val['company_name'],'type'=>$val['type']];
//        }
//        return empty($rows) ? [] : ['' => '输入企业名称/微叮号搜索'] + $rows;
    }
